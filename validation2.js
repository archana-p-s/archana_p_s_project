...
if (p.value.trim()=="" || e.value.trim()=="" || f.value.trim()==""  ||  l.value.trim()=="" || l.value.trim()==""|| u.value.trim()=="" || ph.value.trim()=="")
{
...
{
    let ph=document.getElementById("phone");
    let phl=document.getElementById("phn");
    regex=/^([0-9]{3})[-. ]?([0-9]{3})[-. ]?([0-9]{4})$/;
    //regular expression for checking no of digits ,number and pattern
    if (regex.test(ph.value))
    {
        phl.innerHTML="Valid number!";
        phl.style.color="green";
        return true;
    }
    else
    {
        phl.innerHTML="Invalid!";
        phl.style.color="red";
        return false;  
    }
